...
double dac_lookup(double dacvalue)
{
	...
	// edge cases
	if (dacvalue < dac_map[0])
		return 0;
	if (dacvalue > dac_map[4095])
		return 4095;
...
int main(int argc, char **argv)
{
	...
	int n;
	//start by sending a completely zeroed out surface
	...
	while (1)
	{
		...
		if (n < 1)
		{
			fprintf(stderr, "file empty, done\n");
			break;
		}
		...
		for (int i = 0; i < NUMPOINTS; i++)
		{
			...
			long double Curvevolt = RESISTANCE * allcurves[i] + VOLTAGES[i];
			// Use dacmap to convert voltage to DAC value we should play out with Teensy
			uint16_t dacValueV = dac_lookup(Curvevolt);
			// uint16_t	daczero = dac_lookup(VOLTAGES[i]);
			//dacvalues[i] =(dacValueV-daczero)*50+daczero;
			// if (dacValueV>0)
			// {
			// dacvalues[i] =dacValueV-1;
			// }
			// else{
